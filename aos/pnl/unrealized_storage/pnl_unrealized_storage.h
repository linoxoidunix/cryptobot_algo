...
template <typename Price, typename Qty, typename UpdatePolicy = NetModePolicy>
class NetUnRealizedPnlStorageDefault
    : public PnlUnRealizedStorageInterface<Price, Qty> {
    using UnRealizedPnl =
  ...
  public:
    ...
    void UpdatePosition(common::ExchangeId exchange,
        ...
        (bbo_it != bbo_.end())
                              ? UpdatePolicy::Update(
                  unrealized_pnl_, key,
                  pnl_unrealized_calculator_.Calculate(avg_price, net_qty,
                                                                         bbo_it->second.best_bid,
    ...
    void UpdateBBO(common::ExchangeId exchange, aos::TradingPair tradingPair,
        ...
        (pos_it != position_info_.end())
            ? UpdatePolicy::Update(
                  unrealized_pnl_, key,
                  pnl_unrealized_calculator_.Calculate(pos_it->second.avg_price,
                                                       pos_it->second.net_qty,
                                                       price_bid, price_ask))
            : void();
    }
...
template <typename Price, typename Qty, typename UpdatePolicy = HedgeModePolicy>
class HedgedUnRealizedPnlStorageDefault
    : public PnlUnRealizedStorageInterface<Price, Qty> {
    using UnRealizedPnl =
        PnlUnRealizedStorageInterface<Price, Qty>::UnRealizedPnl;
