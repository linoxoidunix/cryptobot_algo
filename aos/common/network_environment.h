...
struct hash<aos::NetworkEnvironment> {
    std::underlying_type_t<aos::NetworkEnvironment> operator()(aos::NetworkEnvironment pair) const {
        return static_cast<std::underlying_type_t<aos::NetworkEnvironment>>(pair);
    }
};
};  // namespace std
...
class fmt::formatter<aos::NetworkEnvironment> {
  public:
    ...
    constexpr auto format(const aos::NetworkEnvironment& foo, Context& ctx) const {
