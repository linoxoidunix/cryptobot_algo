...
class Signer : public SignerInterface {
  ...
  public:
    ...
    std::pair<bool, std::string> Sign(std::string_view data) override {
        ...
        return {true, Base64Encode(signature,
                                   crypto_sign_BYTES)};  // ← тут вызывается
    }

    std::pair<bool, std::string> SignByLowerCase(std::string_view data) {
        auto [status, result] = Sign(data);
