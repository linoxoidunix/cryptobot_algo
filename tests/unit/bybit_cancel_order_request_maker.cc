...
class MockOrder
    : public aoe::bybit::cancel_order::RequestInterface<MemoryPool> {
  public:
...
TEST(BybitCancelOrderRequestMakerTest, MakeReturnsValidRequestJson) {
    using RequestMaker =
    ...
    EXPECT_CALL(*mock_order, Accept(::testing::_))
        .WillOnce(Return(std::make_pair(
            true, nlohmann::json{{"category", "spot"},
                                 {"symbol", "BTCUSDT"},
    ...
    // Проверка значений
    EXPECT_EQ(result["op"], "order.cancel");
    ...
    EXPECT_EQ(result["args"][0]["category"], "spot");
